...
/* After M5.shutdown( secs ), the whole sketch restarts so it will reenter setup
 * - just like a reboot. This means that all state (variables etc) are lost so
 * you need to use NVS to store anything essential. M5.shutdown doesn't appear
 * to do anything if you have the USB cable connected so we still need the loop
 * with a delay in there. This platform needs some serious work.
 */
...
void setup() {
  ...
  if ((current_time.min == 0) ||
      forced_shutdown) // Clean the display once an hour
  {
    // Clear the display
    M5.EPD.Clear(true);
    cleared = 1;
  ...
  unused_canvas.loadFont("/liquid-crystal.ttf",
                         SPIFFS); // Load font files internal flash
...
int fetch_weather_image(char *url, char *file_name) {
  ...
  if (httpResponseCode > 0) {
    ...
    if (len > 0) {
      ...
      if (len == 0)
        Serial.printf("Weather image fetched successfully.\n");
      else
        return 0;
    } else
      return 0; // fail
...
void get_weather() {
  ...
  sprintf(url, "http://api.weatherapi.com/v1/current.json?key=%s&q=%s",
          weather_api_key, weather_api_location);
  http.begin(url);
  int httpResponseCode = http.GET();
    ...
    if (image_number > 0) {
      if (last_weather != image_number) {
        ...
        if (!SPIFFS.exists(file_name))
          fetch_weather_image(url, file_name);
...
void loop() {
  ...
  */
  ...
  // Current temperature
  if (((int_temp != last_temp) || cleared ||
       ((current_time.hour == 0) && (current_time.min == 0)))) {
    ...
    if (((low_temp == 0) && (high_temp == 0)) ||
        ((current_time.hour == 0) &&
         (current_time.min == 0))) // Load NVR failed or new day
    {
      low_temp = int_temp;
      high_temp = int_temp;
      hi_lo_changes = 1;
    ...
    if ((hi_lo_changes) || cleared) {
      sprintf(msg, "%2.1fC  %2.1fC", (float)low_temp / 10.0,
              (float)high_temp / 10.0);
